...
namespace Semantica
{
    public class Lexico : Token, IDisposable
    {
        ...
        public Lexico()
        {
            Console.Write("Ingrese el nombre del archivo C++ (por ejemplo suma.cpp): ");
            ...
            // Resolver ruta de entrada: si solo se pasó un nombre, buscar en ./cpp/
            string inputPath = nombre;
            if (!Path.IsPathRooted(nombre) && !nombre.Contains(Path.DirectorySeparatorChar.ToString()) && !nombre.Contains(Path.AltDirectorySeparatorChar.ToString()))
            {
                string tryPath = Path.Combine("cpp", nombre);
            ...
            {
                // intentar encontrar en ./cpp/ con el mismo nombre base
                string tryPath = Path.Combine("cpp", Path.GetFileName(nombre));
                if (File.Exists(tryPath))
                    inputPath = tryPath;
            }

            archivo = new StreamReader(inputPath);

            // generar nombres para log y asm usando el nombre base (sin ruta ni extensión)
            string baseNombre = Path.GetFileNameWithoutExtension(Path.GetFileName(nombre));
            string logPath = Path.Combine("logs", baseNombre + ".log");
            string asmPath = Path.Combine("asm", baseNombre + ".asm");
            log = new StreamWriter(logPath);
            asm = new StreamWriter(asmPath);
            asm.AutoFlush = true;
            log.AutoFlush = true;
            linea = 1;
            ccount = 0;
